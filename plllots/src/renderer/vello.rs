...
impl VelloRenderer {
    ...
    pub fn render_to_scene(&mut self, chart: &Chart, scene: &mut Scene) {
        ...
        for primitive in primitives {
            ...
            match primitive {
                crate::primitives::Primitives::Line(line) => line.append_vello(scene, self),
                crate::primitives::Primitives::Text(text) => text.append_vello(scene, self),
            }
        }
        // Render series data
...
impl<'a> AppendVello for crate::primitives::Text<'a> {
    fn append_vello(&self, scene: &mut Scene, vello_render: &mut VelloRenderer) {
        ...
        let text_width = layout.width();
        let transform = Affine::translate((self.coord.x - text_width as f64 / 2.0, self.coord.y));
        for line in layout.lines() {
            for item in line.items() {
                if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
                    let mut x = glyph_run.offset();
